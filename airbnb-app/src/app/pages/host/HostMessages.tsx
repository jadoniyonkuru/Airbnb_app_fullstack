...
import { Search, Send, ChevronLeft, MessageSquare, Loader2, MapPin } from 'lucide-react';
...
const quickReplies = [
  ...
  "WiFi Name: HomeNet | Password: Welcome2026!",
...
function ChatView({ convo, userId }: { convo: UserConversation; userId: string }) {
  ...
  const other = convo.hostId === userId ? convo.guest : convo.host;
  ...
  return (
    <>
      ...
      <div className="p-4 border-b border-[#EBEBEB] flex items-center gap-3">
        <div className="w-10 h-10 bg-[#484848] rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0">
          {other.avatar
      ...
      {/* Property Banner */}
      {convo.listing && (
        <div className="px-4 py-2.5 border-b border-[#EBEBEB] flex items-center gap-3" style={{ background: '#FFF8F9' }}>
          <img
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {isLoading && messages.length === 0 ? (
          ...
          <div className="flex flex-col items-center justify-center h-full gap-2">
            ...
            <p className="text-[#AAAAAA] text-sm">No messages yet.</p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                {!isMe && (
                  <div className="w-7 h-7 bg-[#484848] rounded-full flex items-center justify-center text-white text-xs font-bold mr-2 mt-auto shrink-0">
                    {msg.sender.avatar
...
export function HostMessages() {
  ...
  const filtered = conversations.filter(c => {
    const other = c.hostId === userId ? c.guest : c.host;
    return (
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      <div className="mb-6">
        ...
        <p className="text-[#717171] text-sm">Communicate with your guests.</p>
      </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-full gap-2 p-6 text-center">
                  ...
                  <p className="text-[#AAAAAA] text-xs">Guests will message you once they book your listings.</p>
                </div>
              ) : (
                filtered.map(convo => {
                  const other = convo.hostId === userId ? convo.guest : convo.host;
                  ...
                  return (
                    ...
                    >
                      <div className="w-11 h-11 bg-[#484848] rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0">
                        {other.avatar
                          ? <img src={other.avatar} className="w-full h-full rounded-full object-cover" alt="" />
                          : initials(other.name)}
                      </div>
