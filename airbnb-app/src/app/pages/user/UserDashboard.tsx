  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Bookings', value: loadingBookings ? '—' : String(bookings.length), color: '#FF385C', bg: '#FFF1F3', emoji: '📅' },
          { label: 'Upcoming Trips', value: loadingBookings ? '—' : String(upcomingCount), color: '#00A699', bg: '#E6F7F6', emoji: '✈️' },
          { label: 'Saved Properties', value: '—', color: '#FC642D', bg: '#FFF0EB', emoji: '❤️' },
          { label: 'Reviews Given', value: '—', color: '#484848', bg: '#F0F0F0', emoji: '⭐' },
        ].map((stat, i) => (
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[#EBEBEB] overflow-hidden">
          ...
          <div className="divide-y divide-[#EBEBEB]">
            {loadingBookings ? (
              [1,2,3].map(i => (
                <div key={i} className="p-5 animate-pulse">
                  <div className="flex gap-4">
                    <div className="w-16 h-16 rounded-xl bg-[#F0F0F0] shrink-0" />
              ...
              <div className="p-8 text-center text-[#717171] text-sm">No bookings yet. <Link to="/listings" className="text-[#FF385C] hover:underline">Explore properties</Link></div>
            ) : (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-[#EBEBEB] overflow-hidden">
            ...
            {loadingBookings || bookings.length === 0 ? (
              <div className="p-6 text-center text-[#717171] text-sm">No upcoming trips</div>
            ) : (
              <>
              ...
              {loadingListings ? (
                [1,2,3].map(i => (
                  <div key={i} className="flex items-center gap-3 animate-pulse">
                    <div className="w-12 h-12 rounded-xl bg-[#F0F0F0] shrink-0" />
      ...
      <div className="mt-8 bg-white rounded-2xl border border-[#EBEBEB] p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          {loadingListings ? (
            [1,2,3,4].map(i => (
              <div key={i} className="animate-pulse rounded-xl border border-[#EBEBEB] overflow-hidden">
